...
export default {
  ...
  methods:{
    ...
    myBtnClass2: function( _pSortKey ){
      var itemIndex =  this.selectedKeys.indexOf( _pSortKey );
      return  (  itemIndex > -1 )? 'selected-filter' + ' sf-' + itemIndex : '';
    },
    ...
    reorder:function( _key ){
      let itemIndex = this.selectedKeys.indexOf( _key );
      if( itemIndex  > -1 ) {
    ...
    drawWidget:function( pData, _coachKey='All' ){
      var _this = this;
      var conf = {
        ageRanges: 8,
        maxAge: 64,
        ...
        barMargin: 5,
        paddingRow: 10,
        sortKeys: _this.selectedKeys // null, 'age', 'gender', 'buzzer_count'
      };
      ...
      pData = pData.filter( (d)=>{ 
        if( _coachKey == 'All'){
          ...
          return  ( d['buzzer_coaches'].indexOf( _coachKey ) > -1 )
        }
      } )

      _this.numDataItems = pData.length;

      for( var k in conf.sortKeys ){
        pData.sort(function(x, y){
          return d3.ascending(x[ conf.sortKeys[k]], y[  conf.sortKeys[k] ]);
        })
      }
...
.widget_dots-grid{
  ...
  .filter-option, .sort-option{
    ...
    &.selected-filter{
      ...
      &.sf-1{
        background: darken($lila, 20%);
      }
      &.sf-2{
        background: darken($lila, 35%);
      }
      &.sf-3{
        background: darken($lila, 50%);
      }
    }
  }
